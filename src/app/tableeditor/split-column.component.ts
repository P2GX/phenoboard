...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content class="!pt-4">
      <div class="text-[10px] uppercase font-bold text-slate-400 mb-2 tracking-wider">
        Select Delimiter
      </div>
      <mat-radio-group
      ...
      >
        ...
        <mat-radio-button value="custom" class="compact-radio col-span-2">
          <span [class.text-indigo-600]="selectedDelimiter() === 'custom'" class="font-medium"
            >Custom</span
          >
        </mat-radio-button>
      </mat-radio-group>
      ...
      <div class="mt-4 p-3 bg-slate-900 rounded-lg shadow-inner overflow-hidden">
        <div class="text-[10px] text-indigo-300 uppercase font-black mb-2 tracking-widest">
          Row 1 Preview
        </div>
        <div class="flex-1">
          <div class="text-[9px] text-slate-500 mb-1 uppercase tracking-tighter font-bold">
            Original:
          </div>
          <div
            class="px-2 py-1.5 bg-slate-800 rounded text-xs font-mono text-emerald-400 truncate border border-slate-700"
          >
            {{ data.example }}
          </div>
        </div>
    ...
    <mat-dialog-actions align="end" class="!pb-4 !px-6">
      <button mat-button (click)="cancel()" class="btn-outline-cancel" style="margin-right: 4px;">
        Cancel
      </button>
      <button class="btn-outline-primary" (click)="confirm()" [disabled]="!finalDelimiter()">
        Apply Split
      </button>
    </mat-dialog-actions>
  `,
