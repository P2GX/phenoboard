...
export class CohortWorkflowService {
  ...
  async saveCohort(cohort: CohortData | null): Promise<void> {
    ...
    if (invalidDisease) {
        throw new WorkflowError('MISSING_MOI', `No mode of inheritance specified for ${invalidDisease.diseaseLabel}`);
    }
    await this.configService.saveCohort(cohort);
    }
  ...
  async recordBiocuration(): Promise<void> {
    ...
    if (this.cohortService.getCurationHistory().some(e => e.orcid === event.orcid && e.date === event.date)) {
