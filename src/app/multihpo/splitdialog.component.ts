...
@Component({
  ...
  styles: [
    `
      ...
      ::ng-deep .compact-radio .mdc-radio {
        display: none;
      }
      ::ng-deep .compact-radio .mdc-label {
  ...
  template: `
    ...
    <mat-dialog-content>
      ...
      <mat-radio-group
        ...
        class="delimiter-grid"
      >
        <mat-radio-button value="," class="compact-radio"
          ><span class="radio-card">Comma (,)</span></mat-radio-button
        >
        <mat-radio-button value="/" class="compact-radio"
          ><span class="radio-card">Slash (/)</span></mat-radio-button
        >
        <mat-radio-button value="." class="compact-radio"
          ><span class="radio-card">Period (.)</span></mat-radio-button
        >
        <mat-radio-button value="and" class="compact-radio"
          ><span class="radio-card">"and"</span></mat-radio-button
        >
      ...
      <div class="p-3 bg-slate-50 border rounded-lg">
        <div class="text-[10px] uppercase font-bold text-gray-400 mb-2">
          Resulting Fragments ({{ previewParts().length }})
        </div>
        <div class="flex flex-wrap gap-1.5">
          @for (part of previewParts(); track part) {
            <span
              class="bg-white border border-gray-200 px-2 py-0.5 rounded text-xs text-blue-700 font-medium"
            >
              {{ part }}
            </span>
          }
